...
<div class="page-centered w-70 bg-light p-4 border">
   ...
   <div class="mt-4">
      ...
      <form action="{{route('bankpayments.update', $bankpayment)}}" method="post" enctype="multipart/form-data">
         @csrf
         @method('PATCH')
         ...
         <div class="frow mt-4 stretched auto-col">
            <div class="fancyinput w-20">
               <input type="text" name=' bank' id='bank' value='{{$bankpayment->bank}}' placeholder="Bank name" class="txt-center">
               <label>Bank Name</label>
            </div>
            <div class="fancyinput w-30">
               <input type="text" name='branch' id='branch' value='{{$bankpayment->branch}}' placeholder="Branch name">
               <label>Branch Name</label>
            </div>
            <div class="fancyinput w-20">
               <input type="text" name='challan' id='challan' value='{{$bankpayment->challan}}' placeholder="Challan no.">
         ...
         <div class="frow mt-4 stretched">
            <div class="w-72">
               <div class="fcol fancyinput mr-3">
                  <input type="file" id='scancopy' name='scancopy' placeholder="Scan Copy" class='w-90 m-0 mr-1 p-2' onchange='preview_pic()'>
                  <label for="Name">Upload scanned copy</label>
               </div>
            </div>
            <div class="frow w-24">
               <img src="{{asset('images/vouchers/'.$bankpayment->scancopy)}}" id='preview_img' alt="scan copy" width="100" height="100">
            </div>
         </div>
